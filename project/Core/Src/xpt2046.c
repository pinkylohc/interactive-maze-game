...
extern uint8_t level, restart_flag;
...
/*
 * get a set of adc value after touch
 */
#if   0
static uint8_t XPT2046_ReadAdc_Smooth_XY ( strType_XPT2046_Coordinate * pScreenCoordinate )
{
	...
	} while ( ( macXPT2046_EXTI_Read() == macXPT2046_EXTI_ActiveLevel ) && ( ucCount < 9 ) );
...
void Check_touchkey_setup(void){
	strType_XPT2046_Coordinate strDisplayCoordinate;

	if ( XPT2046_Get_TouchedPoint ( & strDisplayCoordinate, & strXPT2046_TouchPara ) )
	{

			if ( ( strDisplayCoordinate .y > 110 ) && ( strDisplayCoordinate .y < 160 ) ){
				//if ( ( strDisplayCoordinate .x > 95 ) && ( strDisplayCoordinate .x < 145 ) )
				//{
					level = 4;	//custom
				//}
			...
			if ( ( strDisplayCoordinate .y > 220 ) && ( strDisplayCoordinate .y < 270 ) ){
				//if ( ( strDisplayCoordinate .x > 95 ) && ( strDisplayCoordinate .x < 145 ) )
				//{
					level = 5;	//random
				//}
